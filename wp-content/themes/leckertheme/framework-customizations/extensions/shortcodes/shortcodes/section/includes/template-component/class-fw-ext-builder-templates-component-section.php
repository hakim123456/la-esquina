...
class FW_Ext_Builder_Templates_Component_Section extends FW_Ext_Builder_Templates_Component
{
	...
	{
		return __('Sections', 'leckertheme');
	}
	...
	public function _render($data)
	{
		...
		if (empty($html)) {
			$html = '<div class="fw-text-muted">'. __('No Templates Saved', 'leckertheme') .'</div>';
		} else {
			$html =
				'<p class="fw-text-muted load-template-title">'. __('Load Template', 'leckertheme') .':</p>'
	...
	public function _enqueue($data)
	{
		...
		wp_localize_script(
			...
			array(
				'l10n' => array(
					'template_name' => __('Template Name', 'leckertheme'),
					'save_template' => __('Save Section', 'leckertheme'),
					'save_template_tooltip' => __('Save as Template', 'leckertheme'),
				),
			)
		);
	}
	...
	public function _action_ajax_save_template()
	{
		...
		if (empty($template['title'])) {
			$template['title'] = __('No Title', 'leckertheme');
		}
